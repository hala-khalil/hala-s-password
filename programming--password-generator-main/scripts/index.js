...
function generatePassword(length) {
  ...
  for (let i = 0; i<= length; i++){
    let tab = []
    if (shouldIncludeLowerCase()) tab.push (randomLowerCase())
    if (shouldIncludeUpperCase()) tab.push (randomUpperCase())
    if (shouldIncludeNumbers()) tab.push (randomNumber())
    if (shouldIncludeSymbols()) tab.push (randomSymbol())

    console.log(tab)
    motDePass += arrayRandom (tab)
